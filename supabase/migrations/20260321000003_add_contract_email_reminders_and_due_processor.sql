-- ============================================
-- Migration: Add Contract Email Reminder Source Of Truth
-- Description:
--   - Add contract-level email_reminders flag
--   - Keep atomic create/update RPC functions aligned
--   - Add internal-only RPC to fetch due email reminders
-- ============================================
...
SET email_reminders = EXISTS (
  SELECT 1
  FROM public.reminders AS reminders
  WHERE reminders.contract_id = contracts.id
    AND COALESCE(array_length(reminders.notify_emails, 1), 0) > 0
);

DROP FUNCTION IF EXISTS public.create_contract_with_relations(
...
DROP FUNCTION IF EXISTS public.update_contract_with_relations(
  UUID, TEXT, TEXT, TEXT, DATE, DATE, DECIMAL, TEXT, BOOLEAN, TEXT, TEXT, TEXT[], TEXT, TEXT, INTEGER[], TEXT[]
);
...
REVOKE ALL ON FUNCTION public.get_due_email_reminders(TIMESTAMPTZ, INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_due_email_reminders(TIMESTAMPTZ, INTEGER) TO service_role;
